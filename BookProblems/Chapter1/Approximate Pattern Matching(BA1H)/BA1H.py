        ...
        if indices:  # if not null
            for index in indices:
                ...
                for j in range(i + 1, len(kmers) - 1):  # Right-hand side
                    ...
                    if mismatch > d:
                        break
                mismatch += eulerianDistance(kmers[-1], text[index + k * (len(kmers) - i - 1): index + k * (
                            len(kmers) - i - 1) + len(kmers[-1])])
                if mismatch <= d:
...
if __name__ == '__main__':
    INPUT_FILE_NAME = "rosalind_ba1h.txt"
    OUTPUT_FILE_NAME = "ba1h.txt"
